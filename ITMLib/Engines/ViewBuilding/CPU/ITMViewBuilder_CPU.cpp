...
static const std::string TRACE_FILE_BF_PIXEL{TRACE_FILE_PREFIX + "bf_pixels.gz"};

/// Same as the original but also collects a functional trace
inline void filterDepthTrace(float *imageData_out, const float *imageData_in, int x, int y, Vector2i imgDims, std::ostream& os)
{
	float z, tmpz, dz, final_depth = 0.0f, w, w_sum = 0.0f;
	const int pixelId = x + y * imgDims.x;

	// Inst 0 begin
	z = imageData_in[pixelId];
	Trace::writeMemoryInstruction(os, pixelId, 0, "load", "global", "depth", std::addressof(imageData_in[pixelId]));
	// Inst 0 end; global memory load latency cycles

	if (z < 0.0f)
	{
		// Inst 1 begin
		imageData_out[pixelId] = -1.0f;
		Trace::writeMemoryInstruction(os, pixelId, 1, "store", "global", "depth", std::addressof(imageData_out[pixelId]));
		// Inst 1 end; global memory store latency cycles
		return;
	}
	...
	// This can be fully unrolled in hardware
	...
	final_depth /= w_sum;
	Trace::writeComputeInstruction(os, pixelId, 2, 83);
	// Inst 2 end; 15*5 (3 divs, 1 sqrt, and 1 exp) + 1 + 2 + 5 = 83 cycles

	// Inst 3 begin
	imageData_out[pixelId] = final_depth;
	Trace::writeMemoryInstruction(os, pixelId, 3, "store", "global", "depth", std::addressof(imageData_out[pixelId]));
	// Inst 3 end; global memory store latency cycles
}

ITMViewBuilder_CPU::ITMViewBuilder_CPU(const ITMRGBDCalib& calib):ITMViewBuilder(calib)
{
	// Clear trace file
	Trace::clearFile(TRACE_FILE_BF_PIXEL);
}
...
{ 
	...
	if (useBilateralFilter)
	{
		...
		this->DepthFiltering(this->floatImage, view->depth, ss);
		this->DepthFiltering(view->depth, this->floatImage, ss);
		this->DepthFiltering(this->floatImage, view->depth, ss);
		this->DepthFiltering(view->depth, this->floatImage, ss);
		this->DepthFiltering(this->floatImage, view->depth, ss);
	...
	// Write trace to file
	Trace::writeTrace(TRACE_FILE_BF_PIXEL, count, ss);
	count++;
...
/// Same as the original but also collects a functional trace
void ITMViewBuilder_CPU::DepthFiltering(ITMFloatImage *image_out, const ITMFloatImage *image_in, std::ostream& os)
{
	Vector2i imgSize = image_in->noDims;
...
#endif
	for (int y = 2; y < imgSize.y - 2; y++) for (int x = 2; x < imgSize.x - 2; x++)
		filterDepthTrace(imout, imin, x, y, imgSize, os);
}
